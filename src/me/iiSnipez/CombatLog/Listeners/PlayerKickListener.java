...
public class PlayerKickListener implements Listener {
	...
	public void onKickEvent(PlayerKickEvent event) {
		...
		if (plugin.taggedPlayers.containsKey(player.getName()) && plugin.removeTagOnKick && !event.getReason().toLowerCase().contains("spam")) {
			PlayerUntagEvent event2 = new PlayerUntagEvent(player, UntagCause.KICK);
